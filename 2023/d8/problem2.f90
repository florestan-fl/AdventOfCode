...
subroutine ppcm(list, siz, res)
    integer, intent(in) :: siz
    integer, parameter :: MyLongIntType = selected_int_kind (16)
    integer(kind=MyLongIntType), intent(out) :: res
    integer, dimension(siz), intent(in) :: list
    integer(kind=MyLongIntType) :: a, b, p
    integer :: i
...
subroutine problem()
    ...
    integer :: io, isize, i, j, instructionsLen, nbLocs
    ...
    integer, dimension(256) :: currLocs, results
    logical eof
    integer, parameter :: MyLongIntType = selected_int_kind (16)
...
readl:    do while(.not. eof)
    ...
    ! get the starting positions (words ending with 'A')
    do i=1, nbLines
        if (maps(i, 1)(3:3) == 'A') then
            currLocs(i) = i
        else
            exit ! Here the sort reads each word starting by the end so 
            ! all words ending with 'A' are stacked at the start
        end if
    end do
    nbLocs = i - 1
    ...
    do j=1, nbLocs
        ...
        do while (maps(currLocs(j), 1)(3:3) /= 'Z')
            if (instructions(i:i) == 'L') then
                call find(maps, maps(currLocs(j), 2), nbLines, currLocs(j))
            else
